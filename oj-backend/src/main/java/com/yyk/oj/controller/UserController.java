...
import com.yyk.oj.model.dto.user.UserAddRequest;
...
import static com.yyk.oj.service.impl.UserServiceImpl.SALT;
...
public class UserController {
     ...
     */
    @GetMapping("/ranking")
    public BaseResponse<List<UserStatsVO>> getUserRanking(@RequestParam(defaultValue = "10") int limit) {
        List<UserStatsVO> rankingList = userService.getUserRankingList(limit);
    ...
    public BaseResponse<List<UserStatsVO>> getWeeklyUserRanking(@RequestParam(defaultValue = "10") int limit) {
        List<UserStatsVO> rankingList = userService.getWeeklyUserRankingList(limit);
        return ResultUtils.success(rankingList);
    }
    ...
    public BaseResponse<UserStatsVO> getUserStats(@PathVariable Long userId) {
        UserStatsVO userStats = userService.getUserStats(userId);
     ...
     */
    @GetMapping("/stats/my")
    public BaseResponse<UserStatsVO> getMyUserStats(HttpServletRequest request) {
        User loginUser = userService.getLoginUser(request);
    ...
    public BaseResponse<Map<String, Object>> getUserCompletionStats(HttpServletRequest request) {
        User loginUser = userService.getLoginUser(request);

        Map<String, Object> stats = new HashMap<>();

        // 获取用户完成的题目数量
        Integer completedCount = questionSubmitService.countUserAccepted(loginUser.getId());
        Integer totalQuestions = Math.toIntExact(questionService.count());
        ...
        if (totalQuestions > 0) {
            completionRate = Math.round((double) completedCount / totalQuestions * 100.0 * 10.0) / 10.0;
        }
     ...
     */
    @PostMapping("/avatar")
    public BaseResponse<Boolean> updateAvatar(@RequestBody Map<String, String> params,
                                              HttpServletRequest request) {
